"""
=====================================================================================
This script acts as the high-level Human-Machine Interface (HMI). It runs on the host PC 
and communicates with the Master RPi over a USB Serial connection. 

The Host delegates all low-level hardware polling to the microcontrollers and 
focuses entirely on trajectory planning, kinematic state tracking, and safety logic.

Core Architectural Features :
1. Follow-The-Leader (FTL) Kinematics : 
   Translates 2D user clicks on an OpenCV camera 
   feed into absolute Pan/Tilt target angles. When a new click is registered, 
   the targets shift down the link chain, creating a snake-like insertion path.
   
2. Stack-Based Retraction Memory : 
   Every discrete action (a steering click or a lead screw push) 
   is pushed onto a memory stack. Pressing 'R' pops the stack, calculating 
   inverse hysteresis multipliers to unspool the tendons step-by-step 
   without losing absolute coordinate synchronization.
   
3. Asynchronous Safety Handling : 
   A background daemon thread constantly listens to the USB Serial port. 
   If the Master Pico broadcasts a limit switch trigger, the 
   Host intercepts it, halts user input, and automatically executes a pre-mapped 
   escape sequence to back the robot away from the collision.

Commands:
1. W/S keys :
   Move the continuum robot forward/backward for 2 seconds on the lead screw.
2. U/D keys :
   Increase/decrease the tentacle gripper force by 10%.
3. F key : 
   Toggle fullscreen mode for the camera feed.
4. R key :
   "Undo" the most recent move performed.
5. Mouse click on camera feed :
   Move the continuum robot to the desired location by clicking on it. On the first click, 
   the first link moves; on the second click the second link moves and so on.
6. SPACEBAR :
   Stop all the motors.
7. Q key : 
   Quit
...
LEAD_SCREW_TIMEOUT = 2.0    # Matches the Master Pico 2s timeout
...
def switch_to_controller():
    global CONTROL_MODE, controller

    print("[MODE] CONTROLLER ACTIVE")

    pygame.init()
    pygame.display.init()
    pygame.display.set_mode((1,1))  # 🔥 REQUIRED
...
def handle_limit_switch(mcp_id, pin):
    ...
    # Each limit switch is mapped to a certain escape logic.
...
def execute_lead_screw(forward=True):
    ...
    # We do NOT send a stop command. The Master's 2s timeout handles it.
...
def execute_link_step():
    #Bends the continuum links without moving the lead screw.
...
def handle_controller_button(event):
    ...
    if event.button == 0:
        threading.Thread(target=push_trajectory_and_execute, args=(0, -10), daemon=True).start()

    elif event.button == 1:
        threading.Thread(target=push_trajectory_and_execute, args=(10, 0), daemon=True).start()
...
def handle_controller_axis(event):
    global tentacle_pwm, is_moving

    if is_moving:
        return

    if event.axis == 1 and abs(event.value) > 0.6:
        threading.Thread(
            target=execute_lead_screw,
            args=(event.value < 0,),
            daemon=True
        ).start()

    elif event.axis == 4:
        tentacle_pwm = int(100 * abs(event.value + 1)/2)
        print(f"Axis 4 Value: {event.value:.3f} | Calculated PWM: {tentacle_pwm}%")
        tx_command(f"GRIP,0,{tentacle_pwm}\n")
        print(f"Gripper Engaged. Tension: {tentacle_pwm}%")

    elif event.axis == 5:
        tentacle_pwm = int(100 * abs(event.value + 1)/2)
        tx_command(f"GRIP,1,{tentacle_pwm}\n")
        print(f"Gripper Released. Tension: {tentacle_pwm}%")
    ...
    while True:
        ...
        if controller is not None:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.JOYBUTTONDOWN:
                    ...
                    # 🔥 Toggle button (choose one, e.g. button 7)
                    if CONTROL_MODE == "CONTROLLER" and event.button == 7:
                        print("[INPUT MODE] Toggling to Keyboard...")
                        switch_to_keyboard()
                        continue

                    elif CONTROL_MODE == "CONTROLLER":
                        handle_controller_button(event)

                elif event.type == pygame.JOYAXISMOTION:
                    if CONTROL_MODE == "CONTROLLER":
                        handle_controller_axis(event)
        key = cv2.waitKeyEx(1)
        if(key != -1):
            print("KEY:", key)

        # 🔥 Toggle to controller FIRST
        if CONTROL_MODE == "KEYBOARD" and key == ord('t'):
            print("[INPUT MODE] Toggling to Controller...")
        ...
        if CONTROL_MODE == "KEYBOARD":
            ...
            if key == -1:
                ...
                tentacle_pwm -= 10
                if tentacle_pwm <= 0:
                    tx_command(f"GRIP,1,{abs(tentacle_pwm)}\n") 
...
while not done:
    for event in pygame.event.get():
        ...
        # Triggered when an axis moves
        elif event.type == pygame.JOYAXISMOTION:
            print(f"Axis {event.axis} changed to {event.value:.3f}")
